...
class Jdflib
{
    ...
    public static function jdate($format, $timestamp = '', $none = '', $time_zone = 'Asia/Tehran', $tr_num = 'fa')
    {
        ...
        $ts = $T_sec + (($timestamp === '') ? time() : static::tr_num($timestamp));
        $date = explode('_', date('H_i_j_n_O_P_s_w_Y', $ts));
        [$j_y, $j_m, $j_d] = static::gregorian_to_jalali($date[8], $date[3], $date[2]);
        ...
        for ($i = 0; $i < $sl; $i++) {
            ...
            switch ($sub) {
                ...
                case'D':
                    $out .= static::jdate_words(['kh' => $date[7]], ' ');
                    break;

                case'f':
                    $out .= static::jdate_words(['ff' => $j_m], ' ');
                    break;

                case'F':
                    $out .= static::jdate_words(['mm' => $j_m], ' ');
                    break;
                ...
                case'J':
                    $out .= static::jdate_words(['rr' => $j_d], ' ');
                    break;

                case'k';
                    $out .= static::tr_num(100 - (int) ($doy / ($kab + 365) * 1000) / 10, $tr_num);
                    break;

                case'K':
                    $out .= static::tr_num((int) ($doy / ($kab + 365) * 1000) / 10, $tr_num);
                    break;

                case'l':
                    $out .= static::jdate_words(['rh' => $date[7]], ' ');
                    break;
                ...
                case'r':
                    $key = static::jdate_words(['rh' => $date[7], 'mm' => $j_m]);
                ...
                case'v':
                    $out .= static::jdate_words(['ss' => ($j_y % 100)], ' ');
                    break;

                case'V':
                    $out .= static::jdate_words(['ss' => $j_y], ' ');
                    break;
    ...
    public static function jstrftime($format, $timestamp = '', $none = '', $time_zone = 'Asia/Tehran', $tr_num = 'fa')
    {
        ...
        [$j_y, $j_m, $j_d] = static::gregorian_to_jalali($date[7], $date[4], $date[3]);
        $doy = ($j_m < 7) ? (($j_m - 1) * 31) + $j_d - 1 : (($j_m - 7) * 30) + $j_d + 185;
        $kab = (((($j_y % 33) % 4) - 1) == ((int) (($j_y % 33) * 0.05))) ? 1 : 0;
        $sl = strlen($format);
        $out = '';
        for ($i = 0; $i < $sl; $i++) {
            ...
            switch ($sub) {
                ...
                /* Day */ case'a':
                $out .= static::jdate_words(['kh' => $date[6]], ' ');
                break;

                case'A':
                    $out .= static::jdate_words(['rh' => $date[6]], ' ');
                    break;
                ...
                $key = static::jdate_words(['rh' => $date[6], 'mm' => $j_m]);
    ...
    /*	F	*/
    public static function jmktime(
        $h = '',
        $m = '',
        $s = '',
        $jm = '',
        ...
        if ($h === '') {
            ...
            [$h, $m, $s, $jm, $jd, $jy] = explode('_', static::tr_num($h.'_'.$m.'_'.$s.'_'.$jm.'_'.$jd.'_'.$jy));
            if ($m === '') {
                return mktime($h);
            } else {
                if ($s === '') {
                    ...
                    if ($jm === '') {
                        ...
                        $jdate = explode('_', static::jdate('Y_j', '', '', $timezone, 'en'));
                        if ($jd === '') {
                            [$gy, $gm, $gd] = static::jalali_to_gregorian($jdate[0], $jm, $jdate[1]);

                            return mktime($h, $m, $s, $gm);
                        } else {
                            if ($jy === '') {
                                [$gy, $gm, $gd] = static::jalali_to_gregorian($jdate[0], $jm, $jd);

                                return mktime($h, $m, $s, $gm, $gd);
                            } else {
                                [$gy, $gm, $gd] = static::jalali_to_gregorian($jy, $jm, $jd);
    ...
    {
        [$jm, $jd, $jy] = explode('_', static::tr_num($jm.'_'.$jd.'_'.$jy));
    ...
    {
        [$gy, $gm, $gd] = explode('_', static::tr_num($gy.'_'.$gm.'_'.$gd));/* <= Extra :اين سطر ، جزء تابع اصلي نيست */
    ...
    {
        [$jy, $jm, $jd] = explode('_', static::tr_num($jy.'_'.$jm.'_'.$jd));/* <= Extra :اين سطر ، جزء تابع اصلي نيست */
        if ($jy > 979) {
            $gy = 1600;
            $jy -= 979;
        } else {
